...
over tick-derived 1-minute candles, blends in the ChainVector Probability
Engine, passes a deep stack of entry vetoes, and places real limit orders
with stop-loss / take-profit / reversal-exit management.

**[ChainVector](https://chainvector.com) is the sole derived-signals
provider.** Kalshi is used only as the execution venue (plus its own
orderbook/trade tape), and Coinbase/OKX/Deribit index reads survive only as
the raw settlement-price oracle fallback chain. Every other market signal —
probabilities, momentum, liquidations, order flow, funding, whales, quote
stability, regime — comes from the ChainVector API.
...
| Prediction bid-price stability | `/predictions/stability` | Entry veto when the Kalshi quote is repricing hard against the side (cross-checks the local bid-stability gate) |
...
# edit .env.local: add your Kalshi API key ID, the path to your Kalshi RSA
# private key PEM, and your ChainVector API key
```
...
| `--bankroll` | `500` | Starting bankroll in USD (replaced by the live Kalshi balance at startup when available). |
...
| Flag | Default | Description |
|---|---|---|
| `--no-cv-stab-veto` | veto **on** | Disable the `/predictions/stability` quote-stability veto (blocks while the Kalshi quote is repricing hard against our side). |
...
| Flag | Default | Description |
|---|---|---|
| `--taker-flow-veto-enabled` / `--no-taker-flow-veto` | **on** | Block when recent Kalshi taker aggression is heavily against the bet near the strike. |
...
| `--bid-stab-veto-enabled` / `--no-bid-stab-veto` | **on** | Require our side's Kalshi bid to be stable/rising over the lookback (cross-checked by the ChainVector stability veto). |
...
| Flag | Default | Description |
|---|---|---|
| `--no-orderbook-signal` | recording **on** | Disable Kalshi orderbook depth recording (audit-only). |
| `--no-trade-flow-signal` | recording **on** | Disable Kalshi recent-trades recording (taker aggression, audit-only). |
| `--trade-flow-lookback-n` | `20` | Recent Kalshi trades summarized per poll. |
...
python-service/
  ...
  kalshi_orderbook.py# Kalshi book/trade-tape snapshots (native, execution venue)
...
- **Never commit `.env.local` or your PEM** — `.gitignore` covers both.
- Logs, audit JSONL and caches go to the first writable of
  `$BTC15M_DATA_DIR` → package dir → `~/.kalshi_btc_15m_cv` → tmp, so the
...
- This trades real money on Kalshi. Verify with `--dry-run` and a small
  bankroll first.
