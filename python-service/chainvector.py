...
API (https://chainvector.com/docs). Kalshi remains the execution venue (and
its own orderbook/trades are read natively); Coinbase/OKX/Deribit index reads
remain only as the raw settlement price oracle. Everything else — momentum,
probabilities, liquidations, order flow, funding, whales, prediction-market
stability/edge — is ChainVector.
...
Endpoints wrapped here:
  ...
  • stability(market_id)      — /predictions/stability: Kalshi YES-quote
  ...
  • edge(market_id)           — /predictions/edge: ChainVector model prob vs
                                the live Kalshi quote (model_prob, buy_edge,
                                sell_edge). Recorded signal.
  • prediction_markets/quotes/trades/results — Kalshi series metadata,
...
class ChainVectorClient:
    ...
    def stability(self, market_id: str) -> Optional[dict]:
        """Quote stability & momentum for one Kalshi market. Raw per-window
        dict {"30s": {...}, "1m": {...}, "5m": {...}} where each window has
        stability_score (0..100, 100 = frozen tight quote), momentum_score
        (−100..+100 signed YES-mid repricing speed), churn, consistency,
        avg_spread. None if the market has no live quotes."""
        return self._cached(
    ...
    def edge(self, market_id: str) -> Optional[dict]:
        """Model-vs-market edge for one Kalshi market at full MC precision:
        {model_prob, market_prob, yes_bid, yes_ask, edge, buy_edge,
        sell_edge, minutes_to_close}. Recorded signal."""
    ...
    def prediction_markets(self, series: str) -> Optional[list]:
        """Market metadata for a Kalshi series (discovery / research)."""
        return self._cached(
            "pred_markets", series, 60.0,
            lambda: self._get("/predictions/markets",
                              {"venue": "kalshi", "series": series}))

    def prediction_quotes(self, market_id: str, limit: int = 50) -> Optional[list]:
        """Recent YES top-of-book history for one Kalshi market."""
        return self._cached(
    ...
    def prediction_trades(self, series: str = "KXBTC",
                          limit: int = 100) -> Optional[list]:
        """Recent taker tape across a Kalshi series (order-flow research)."""
        return self._cached(
    ...
    def prediction_results(self, series: str = "KXBTC15M",
