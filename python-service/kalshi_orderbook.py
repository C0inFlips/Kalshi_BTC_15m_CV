"""
kalshi_orderbook.py — Snapshot Kalshi orderbook depth + recent trade flow
...
def _to_int(count_str) -> int:
    """Kalshi count_fp '36.00' → 36 (truncates fractional contracts from partial fills)."""
    try:
...
def summarize_orderbook(raw: dict) -> dict:
    """Compute aggregate metrics from a raw Kalshi orderbook response.
...
def summarize_trades(raw: dict, lookback_n: int = 20) -> dict:
    """Compute trade flow aggression metrics from recent Kalshi trades.
...
KalshiGetFn = Callable[..., Awaitable[dict]]


async def fetch_orderbook(ticker: str, kget: KalshiGetFn) -> Optional[dict]:
...
async def fetch_recent_trades(ticker: str, kget: KalshiGetFn,
