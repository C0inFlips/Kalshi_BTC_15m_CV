...
Computes the probability that BTC ends above a given strike at the Kalshi
...
unchanged (the estimator mapping is documented below):
  ...
  hours_to_deribit_expiry   — equals hours_to_kalshi_expiry (exact TTE!)
...
Public functions:
  compute_terminal_prob(spot, strike, kalshi_close_dt) -> dict
...
def compute_terminal_prob(spot: float, strike: float,
                          kalshi_close_dt: datetime,
                          *, currency: str = "BTC") -> dict:
    """Compute the ChainVector ensemble probability that BTC > strike at
    kalshi_close_dt. Returns a flat dict (legacy-compatible keys, see module
    ...
    out: dict = {
        "spot":   spot,
        "strike": strike,
        "kalshi_close_dt": kalshi_close_dt.isoformat(),
        "computed_at":     now.isoformat(),
        "prob_source":     "chainvector",
        "error":  None,
    }
    tte_s = (kalshi_close_dt - now).total_seconds()
    T_years = max(0.0, tte_s / (365.25 * 24 * 3600))
    out["hours_to_kalshi_expiry"] = T_years * 365.25 * 24
    out["hours_to_deribit_expiry"] = out["hours_to_kalshi_expiry"]  # exact TTE
    ...
    close_ts_ms = int(kalshi_close_dt.timestamp() * 1000)
...
if __name__ == "__main__":
    ...
    for offset_pct in [-0.10, -0.05, 0.00, +0.05, +0.10]:
        ...
        if r.get("error"):
            ...
            continue
        print(f"  TTE: {r['hours_to_kalshi_expiry']*60:.1f} min (exact close_ts)")
