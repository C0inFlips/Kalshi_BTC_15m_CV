"""
trade_daemon.py — Autonomous Kalshi BTC 15m trading daemon (ChainVector edition)
...
Signals architecture:
  • Kalshi          — execution venue + its own orderbook/trade tape (native)
  • Coinbase/OKX/Deribit index — RAW settlement price oracle only (fallback chain)
  • ChainVector     — SOLE derived-signals provider: probability engine
...
Usage:
  ...
  python trade_daemon.py --bankroll 500
"""
...
# Windows consoles default to cp1252 and choke on chars like Δ / ¢ in our log lines.
...
# Terminal probability — ChainVector Probability Engine (six-estimator
# ensemble on tick-derived 1m bars, exact close_ts TTE). Weighted into
# combined_p via --ev-tp-weight — the probability engine IS the EV weight.
...
# feeds). snapshot_new_signals() never raises; rrm_evaluate/latest_perp_mid —
# Reversal-Risk Monitor; cv_composite — flip-probability composite;
# context_signals — recorded regime/volatility/risk-index bundle.
...
# cv_lead via the same singleton — one cache, one rate-limit budget).
_cv_client = get_cv_client()
# Kalshi orderbook + recent trade flow — record-only signals added 2026-05-28.
# Audit logs include orderbook depth snapshots + trade aggression metrics on
# every poll for retrospective analysis. After 1-2 days of data we can
# correlate features (imbalance, spread, aggression) with WIN/LOSS outcomes
# and promote any predictive ones to gates.
from kalshi_orderbook import fetch_orderbook, fetch_recent_trades
...
# Structured JSONL audit log — one row per signal evaluation + trade + settle.
...
KALSHI_BASE    = KALSHI_HOST + KALSHI_PREFIX
...
# BTC15M_DATA_DIR env override -> package dir -> ~/.kalshi_btc_15m_cv -> tmp.
...
def _build_headers(method: str, path: str) -> dict:
    ...
    sign_path = path if path.startswith(KALSHI_PREFIX) else KALSHI_PREFIX + path
    sig = pk.sign(
...
# ── 2026-07-02: KALSHI BID-STABILITY ENTRY GATE ─────────────────────────────
...
async def _kdelete(path: str) -> dict:
    """DELETE — used to cancel resting orders. Returns {} on 404 (already
    gone/filled) so callers can treat 'order no longer cancelable' as success."""
    hdrs = _build_headers("DELETE", path)
    async with httpx.AsyncClient(timeout=15) as c:
...
def _order_fill_count(order: dict) -> int:
    """Contracts filled on an order. Per Kalshi GetOrders spec the canonical
...
# Kalshi order status enum (GetOrders spec): only these three values exist.
...
async def _place_resting_tp_for_trade(trade: dict, *, take_profit_cents: int,
    ...
    Never raises. NOT perp-gated — the entry perp is unknown for a recovered
    ...
    side = trade["side"]
    try:
        # 2026-06-18: V2 migration (legacy POST /portfolio/orders deprecated → 410).
...
async def monitor_recovered_position(session: "Session", audit, trade: dict,
    ...
    RRM / patient-topup) — a recovered position just needs SL + TP protection.
    ...
    try:
        ...
        log.warning(f"  [RECOVERED-SL] {ticker}: no close_time — skipping monitor")
        return
    ...
    while True:
        ...
        if mins_remaining < sl_disable_late_mins:
            ...
            log.info(f"  [RECOVERED-SL] {ticker}: {mins_remaining:.1f}min left — "
            ...
            try:
                ...
                if _rst:
                    ...
                    if (trade["contracts"] <= 0
                        ...
                        log.warning(f"  [RECOVERED-SL] {ticker}: TP fully filled — closed.")
                        trade["resting_tp_id"] = None
        ...
        # ── STOP-LOSS HIT ── cancel TP (confirmed) before selling.
        if trade.get("resting_tp_id"):
            if not await _cancel_order_confirmed(trade["resting_tp_id"]):
                log.error(f"  [RECOVERED-SL] {ticker}: could not confirm TP cancel — "
        ...
        actual = await get_kalshi_position_count(ticker)
        if actual == 0:
            log.warning(f"  [RECOVERED-SL] {ticker}: Kalshi shows 0 contracts — "
        ...
        if trade["contracts"] <= 0:
            session.pending.pop(window_id, None)
            return
        # Partial fill — scale cost/net_win and keep monitoring the remainder.
...
class Session:
    def __init__(self, bankroll: float,
        ...
        # ── 2026-06-11: ADAPTIVE BANKROLL (risk throttle) ────────────────────
        ...
        # on demonstrated win-rate recovery. In-memory only — a manual
        ...
        # ── 2026-06-09: Rolling-WR adaptive regime throttle ─────────────────
    ...
    def limit_hit(self) -> Optional[str]:
        ...
        # (1.5x) — barely more than ONE stopped loss — and paused an 86%-WR
    ...
    def _adaptive_audit(self, event: str, reason: str):
        ...
        log_fn(f"[ADAPTIVE-BR] {event} — {reason}")
    ...
    def _update_defensive_state(self, triggering_outcome_was_win: bool = False):
        ...
        if self.defensive_mode:
            if triggering_outcome_was_win:
                ...
                log.info(
                    f"[REGIME] Exiting DEFENSIVE MODE — first win after entry "
...
def _normalize_market(m: dict) -> dict:
    ...
    current Kalshi schema's *_dollars strings. Mutates and returns the input dict."""
    def _to_cents(v):
        if v is None:
            return None
        try:
...
async def fetch_market() -> Optional[dict]:
    ...
    try:
        data = await _kget("/markets", {"series_ticker": "KXBTC15M", "limit": 25})
...
async def get_kalshi_position_count(ticker: str) -> Optional[int]:
    """Query Kalshi for the actual position count on a specific ticker.
    ...
    SELL IOC orders (stop-loss) so we never accidentally over-sell — which
    Kalshi would either reject OR allow as a short position (very bad).

    Why this matters:
      • Our in-memory `pos["contracts"]` is set from `fill_count` returned by
        Kalshi at BUY time. Almost always accurate, but:
      ...
      • Querying actual position right before SELL gives us ground truth;
        we use `min(in_memory, kalshi_actual)` for the sell quantity.
    """
    try:
        data = await _kget("/portfolio/positions", {"ticker": ticker, "limit": 5})
        positions = data.get("market_positions") or data.get("positions") or []
    except Exception as e:
        log.debug(f"  get_kalshi_position_count({ticker}) failed: {e}")
        return None
...
async def get_kalshi_position_avg_cents(ticker: str) -> Optional[int]:
    """Authoritative AVERAGE cost basis (cents/contract) for a ticker position,
    from Kalshi's market_exposure ÷ count. Returns None on failure / no position.
    ...
    patient-top-up rounding (observed 2026-06-15: tracked 80¢ vs actual 75.36¢,
    pushing the TP target to 90¢ instead of ~85¢). Mirrors recover_open_
    ...
    *_dollars string variant)."""
    try:
        ...
        log.debug(f"  get_kalshi_position_avg_cents({ticker}) failed: {e}")
        return None
    for pos in positions:
        if pos.get("ticker") != ticker:
            continue
...
# (Coinbase branch only) so the series is pure Coinbase, the Kalshi BTC
...
async def run_signal(market: dict, bankroll: float,
                       ...
                       # gap/persist/TP point at extremes — in lock-in markets
    ...
    # Compute the last 6 closed 5-min returns (% change) — captured in the
    ...
    # Gates — all thresholds imported from run_backtest so research_loop
    ...
    # risk:reward profile — paying 65-73¢ for $1 payout) were stuck on the
    ...
    if is_golden and golden_no_hurst:
        hurst_ok = True
    # Golden zone widening (2026-05-27): expanded from 3-12 → 2-14 min.
    #
    ...
    #   • The research was generated with Markov-only signals. Since then
    ...
    #   • Late side (2 min) catches near-close lock-in trades that the
    #     late-window-sure tier can confirm via TP ≥ 0.85.
    ...
    # Adaptive near-strike threshold: when Markov gap is weak (≤ 0.20),
    # require a larger buffer from strike. Near-strike trades on thin
    # signals are noise — BTC easily moves $30-80 in 5-10 min. Strong
    # signals get the original 0.02% lenient threshold preserved.
    NEAR_STRIKE_STRONG_GAP = 0.20
    NEAR_STRIKE_DIST_STRONG = 0.02   # |dist| ≥ 0.02% when gap ≥ 0.20
    NEAR_STRIKE_DIST_WEAK   = 0.10   # |dist| ≥ 0.10% when gap < 0.20
    ...
    if not dist_ok:      reasons.append(
        f"near-strike noise (dist={dist_pct:+.4f}%, need ≥{dist_threshold:.2f}% with gap={gap:.3f})"
    )

    # Last-bar adverse-momentum gate (added 2026-05-27 PM after a NO @ 73¢
    # loss where the LAST 5m bar was +0.14% — a clear bounce just starting
    # — but the overall 6-bar momentum was still bearish so the existing
    ...
    # AND TP align at extremes (gap≥0.30 AND TP-direction at HIGH-CONV-grade
    # ≥0.85 / ≤0.15), the recent 5m bar that "opposes" direction is almost
    ...
    # (gap=0.50, p_yes=0, TP=0.034 for NO at 97¢) is a textbook example.
    last_bar_ok = True
    ...
    if last_bar_adverse_threshold > 0 and recent_5m_pct and not extreme_signal_bypass:
        ...
        if yes_lean_for_dir and last_bar_pct < -last_bar_adverse_threshold:
            ...
            reasons.append(
                ...
                f"threshold ±{last_bar_adverse_threshold:.2f}%)"
            )
...
async def get_live_bankroll() -> Optional[float]:
    """Kalshi /portfolio/balance current schema:
...
async def recover_open_positions(session: "Session") -> int:
    """On startup, query Kalshi for any open KXBTC15M positions and rebuild the
    ...
    Defensive about field shape because Kalshi's positions schema has multiple
    legacy/new field names (position vs position_fp, market_exposure vs
    market_exposure_dollars). Only recovers active (not yet settled) KXBTC15M
    markets — skips manually-opened positions on other series.
    """
    try:
        data = await _kget("/portfolio/positions",
                           {"count_filter": "position", "limit": 50})
    except Exception as e:
        log.warning(f"Position recovery: failed to query Kalshi: {e}")
        return 0
    ...
    for pos in positions:
        ...
        if not ticker.startswith("KXBTC15M-"):
            # Skip non-15m-BTC markets — could be manual user trades
            continue
        ...
        try:
            ...
            if m.get("result") in ("yes", "no"):
                # Already settled — Kalshi has already credited/debited the
        ...
        net_win_total     = contracts - cost_total  # $1/contract payout − cost
        ...
        recovered += 1
        log.info(
            f"RECOVERED — {ticker} {side} {contracts}c @ ~{avg_price_cents}¢ "
...
def _log_active_params():
    ...
    log.info("─── Active strategy params ──────────────────────────────────────")
    drift = []
...
def _is_orderbook_lockin(*,
    ...
    """Detect 'lock-in' market signature in the Kalshi orderbook.

    A lock-in is when the orderbook screams a directional consensus:
      • Tight spread (≤ `spread_max` cents) — market makers agree on price
      • Top-of-book price on OUR trade direction at extreme (≥ `price_min`)
      • Markov gap also strong (≥ `gap_min`) — internal model agrees

    When all three hold, we have orthogonal confirmation from a SECOND data
    source (Kalshi's own market microstructure) that the trade direction is
    locked in. Used to bypass TP-based gates that fail because Deribit's
    13h IV undershoots 15min binary probability — the orderbook captures the
    ...
    if spread > spread_max:
        return False, f"spread={spread}¢ > {spread_max}¢"
    ...
    if top_bid < price_min:
        return False, f"top {new_rec} bid {top_bid}¢ < {price_min}¢"
    if gap < gap_min:
...
def _is_late_window_sure(*,
                         ...
                         # 2026-05-28 PM: mirrors HC's lock-in TP bypass —
    ...
    `late_window_price_max` (default 89¢) at a more lenient EV floor.
    """
    ...
    if limit_price > late_window_price_max:
        return False, f"price {limit_price}¢ > late-cap {late_window_price_max}¢"
    # TP check — bypass-able when orderbook lock-in is confirmed (mirrors HC).
    ...
    # Markov must at least lean our direction (no gap requirement — that's the
    ...
    # Recent 5min momentum confirmation. We look at the most recent 5min bin —
...
def _is_high_conviction(*,
                        ...
                        # of 9-day audit showed strict HC (gap≥0.40, tp≥0.90)
                        # combined with |dist|≥0.25% gave 13/13 wins (100% WR,
    ...
    strongly at extremes — used to lift the price cap for markets that are
    ...
    earlier entry — the reversal risk that the 10-min cap protects against
    is small when both Markov and TP say >99% same direction.

    Returns (is_high_conv, reason_if_not). When True, the caller may bid up
    to `high_conv_price_max` (default 97¢) but **must** show positive EV at
    ...
    if limit_price > high_conv_price_max:
        return False, f"price {limit_price}¢ > high-conv cap {high_conv_price_max}¢"
    ...
    # TP check — bypass-able when orderbook lock-in is confirmed.
    if tp_cached is None or tp_cached.get("error"):
        if not orderbook_lockin_bypass:
            return False, "TP unavailable"
    else:
    ...
    # Recent 5m momentum confirmation — don't bet 95+¢ on NO if BTC just
    ...
    # ── 2026-06-03 NEW: TIER 4 — LOW-HURST HC veto ────────────────────────
    ...
    # easily flip the contract. 9-day backtest showed |dist|≥0.25% combined
...
def _is_high_conv_directional(*,
    ...
    information flow, not unpredictability — exactly the situation where
    high-conv signals deserve to trade despite the vol gate firing.

    Two-tier check:
      1. BTC must be meaningfully far from strike (≥ distance_min_pct%) AND
      2. EITHER recent 5m return confirms direction (≥ momentum_min_pct% in
         our favor), OR distance is strong (≥ strong_distance_pct%, in which
         case the distance itself is the directional signal — useful when
    ...
    if rec == "NO":
        if distance_pct > -distance_min_pct:
            return False, (f"BTC only {distance_pct:+.3f}% from strike "
                           f"(need ≤ -{distance_min_pct:.3f}% for NO bypass)")
    else:  # YES
        if distance_pct < distance_min_pct:
            return False, (f"BTC only {distance_pct:+.3f}% from strike "
                           f"(need ≥ +{distance_min_pct:.3f}% for YES bypass)")

    abs_distance = abs(distance_pct)
    # Strong-distance fast path: BTC has clearly moved into our zone, distance
    # alone is the directional signal — no 5m momentum required. Captures
    ...
    if rec == "NO":
        if last_5m > -momentum_min_pct:
            return False, (f"5m momentum {last_5m:+.3f}% "
                           f"(need ≤ -{momentum_min_pct:.3f}% or distance ≥ {strong_distance_pct:.3f}%)")
    else:
        if last_5m < momentum_min_pct:
            return False, (f"5m momentum {last_5m:+.3f}% "
                           f"(need ≥ +{momentum_min_pct:.3f}% or distance ≥ {strong_distance_pct:.3f}%)")
...
def _is_late_window_directional(*, mins_left: float, gap: float, persist: float,
    ...
    """Late-window directional vol bypass — looser than high-conv but late only.
    ...
    Two-tier momentum check (mirrors high-conv-directional):
      1. distance ≥ distance_min (required floor — BTC actually positioned)
      2. EITHER |distance| ≥ strong_distance (no momentum required — distance
         alone is enough when BTC has clearly moved past the strike)
         OR recent 5m return confirms direction with magnitude ≥ momentum_min
    ...
    if new_rec == "NO":
        if dist_pct > -late_dir_distance_min:
            return False, (f"BTC only {dist_pct:+.3f}% from strike "
                           f"(need ≤ -{late_dir_distance_min:.3f}%)")
    else:
        if dist_pct < late_dir_distance_min:
            return False, (f"BTC only {dist_pct:+.3f}% from strike "
                           f"(need ≥ +{late_dir_distance_min:.3f}%)")
    ...
    if new_rec == "NO" and last_5m > -late_dir_momentum_min:
        return False, (f"5m momentum {last_5m:+.3f}% "
                       f"(need ≤ -{late_dir_momentum_min:.3f}% or |dist|≥{late_dir_strong_distance:.3f}%)")
...
# ── Safety hedge helpers (2026-05-31) ──────────────────────────────────────────
def compute_hedge_target(yes_contracts: int, yes_entry_cents: int,
    ...
    Math:
      YES loss when SL fires ≈ sl_loss_cents per contract (conservative).
      OPPOSITE gain at settle = opposite_settle_assumed - (opposite_ask_cents/100).
      To zero out: M = N × yes_loss_per_contract / opposite_gain_per_contract.
...
def hedge_eligible(tier: str, side: str, entry_cents: int,
    ...
    skip — important for understanding why hedges aren't attaching.
    """
...
def smart_flip_eligibility(tier: str, primary_side: str, mins_remaining: float,
    ...
    Gates:
      ...
      - Primary tier is HC or STRONG (entries 70-88¢ — the sweet spot)
      - Enough time remains to fill, monitor, and exit
      - Opposite bid in the sweet-spot band [50, 75]¢ (BTC clearly past strike
...
async def _hedge_topup_inline(hedge_state: dict, current_yes_filled: int,
    ...
    count — so partial YES fills get partially-sized hedges automatically.
    ...
    if current_yes_filled <= 0:
        return
    # Fetch fresh orderbook (this is a Kalshi REST call — ~100-300ms)
    try:
        mkt_data = await _kget(f"/markets/{ticker}")
...
async def main_loop(dry_run: bool, bankroll: float,
                     ...
                     # side) shows the Kalshi quote repricing hard against us.
                     ...
                     # direction — the 6s blip was clearly counter-trend
                     ...
                     # losers dipped $459/$606 — a $400 cap clips 0 winners and
                     ...
                     # 2026-05-30: HC trades enter deep ITM (87-89¢ NO) where
                     # downside is asymmetric. Audit of all 3 HC SL exits showed
                     # the bid blew past BOTH the 30¢ and 25¢ triggers BETWEEN
                     # polls — the daemon never observed an intermediate price.
                     ...
                     # reversals before the Kalshi bid catches up — leading
                     # rather than lagging the SL trigger.
                     # 2026-05-30 (initial): 0.30%/60s — too conservative,
                     #   ZERO fires in first 10h despite 3 SL losses.
                     # 2026-05-30 (tightened): 0.20%/30s — more sensitive
                     ...
                     # If YES wins → NO settles 0 (cost = hedge premium).
                     # If YES SL fires → hold NO to settlement; NO gain offsets
                     ...
                     # Math fails for LATE-SURE (very deep ITM) — gate excludes.
                     ...
                     # (−$402 total), blocks 3 small wins (+$37), net +$365.
                     ...
                     # drift. The market never repriced (bid stayed 84-96¢) so
                     ...
                     # 2026-06-03: TIER 4 — LOW-HURST HC veto.
                     ...
                     # strict HC (gap≥0.40, persist≥0.95, tp≥0.90) + |dist|≥0.25
                     ...
                     # Hypothesis: when Kalshi market dips into the 40-55¢ band
                     ...
                     # 7-day counterfactual: replacing early ~85¢ entries with
                     # ~50¢ entries on the same 70 winning windows would have
                     # added +$1,091 in PnL.
                     # Default OFF — opt-in via --fade-bounce-enabled.
                     ...
                     # ── 2026-06-16: OKX-momentum confidence boost ───────────
                     okx_boost_enabled: bool  = False,
                     okx_boost_weight:  float = 0.06,   # max ~±6pp nudge
                     ...
                     # When all three hold (spread≤max, top-of-book ≥ price_min,
                     # Markov gap ≥ gap_min, direction-aligned), Kalshi's own
                     # microstructure has confirmed the trade direction at
                     # extreme conviction. Used to (a) bypass HC's TP threshold
                     # and (b) raise the LATE-SURE cap by 1¢ (98 → 99).
                     orderbook_lockin_enabled:        bool = True,
                     orderbook_lockin_spread_max:     int  = 2,
                     # 2026-05-28 PM3: lowered 90 → 85. 90 was still strict
                     # in moderate markets (e.g., tonight's 22:30 window:
                     # NO @ 80¢ with no_top_bid=75¢ — clearly directional
                     # consensus but not "extreme"). At 85¢ market still
                     # implies 85% confidence; combined with gap≥0.20 and
                     # spread≤2¢ provides cross-source confirmation.
                     orderbook_lockin_price_min:      int  = 85,
                     # 2026-05-28 PM2: lowered 0.30 → 0.20 to match the new
                     ...
                     # this, lock-in HC fires only 1 contract — defeats the
                     # purpose. Matches LATE-SURE's 1.5% default.
                     hc_lockin_min_stake_pct:         float = 0.015,
                     # 2026-05-28 PM3: NEW — allow STRONG-FLOOR tier to bypass
                     # the Hurst gate (previously only HC/LATE-SURE could).
                     # Justified when strong-floor's other gates (gap≥0.13,
                     # price≤88, TP-meaningful, momentum, timing) hold.
                     ...
                     # 2026-05-28: raised 96 → 98 after 3-day audit showed
                     # EVERY late-window lock-in candidate (104 polls) was at
                     # 97-100¢ ask. Cap at 96 made the tier unreachable.
                     late_window_price_max:   int   = 98,
                     # 2026-05-28: lowered 0.85 → 0.75. Deribit IV is for 13h+
                     # options applied to a 3-min binary — BS underestimates
                     ...
                     # 2026-05-28: NEW — allow LATE-SURE to bypass the high-vol
                     ...
                     # ── 2026-06-15: MIN ENTRY SIZE (standard tier) ──────────
                     ...
                     # ── 2026-06-15: GOLDEN-ZONE EXPANSION (backtest-validated) ──
                     golden_price_lo: int = 65,
                     golden_price_hi: int = 73,
                     golden_no_dist:  bool = False,
                     golden_no_hurst: bool = False,
                     # 2026-05-28 PM2: lowered 0.30 → 0.20. HC's gap was
                     # historically 0.30 (strict — extreme Markov agreement).
                     # Today's audit shows many windows had sustained gap
                     # 0.20-0.30 with strong direction and orderbook lock-in
                     # but were blocked from HC. With orderbook + persist≥0.90
                     ...
                     # 2026-05-28 PM: lowered 0.85 → 0.80. Same Deribit-IV
                     ...
                     # TP=0.83-0.84 (gap 0.49, p_yes 0.99) — clearly
                     ...
                     # ── 2026-06-09: Rolling-WR adaptive throttle ────────────
                     ...
                     # fresh EV at current ask) — the adverse-selection guard.
                     ...
                     # ── 2026-06-15: PERP-CONFIRMED TAKE-PROFIT ──────────────
                     ...
                     # ── 2026-06-15: RESTING-ORDER TAKE-PROFIT ───────────────
                     # Places a real resting GTC limit sell at entry+TP on fill,
                     # captured by Kalshi the instant the bid touches it (no
                     ...
                     # ── 2026-06-16: RESTING-TP RE-ENTRY (live test) ─────────
                     ...
                     # Re-entry only ever happens AFTER a full sell — partial
                     ...
                     # ── 2026-06-16: STOP-LOSS RE-ENTRY (live test) ──────────
                     ...
                     # ── 2026-06-15: HIGH-PRICE CEILING TP ───────────────────
                     # For high-price "sure" entries (>= min_cents, e.g. 89), a
                     # +take_profit_cents target would exceed 99¢ and never
                     ...
                     # ── 2026-06-27: HIGH-RISK TIGHT-TP (conditional) ────────
                     ...
                     # ── 2026-06-12: PERP-MOMENTUM ENTRY VETO ────────────────
                     # Skip entries when the Kalshi perp 30s tape moves against
                     ...
                     # 2026-07-02: KALSHI BID-STABILITY ENTRY VETO
                     ...
                     # ── 2026-06-14: BOOK-SKEW ENTRY VETO (golden-only) ──────
                     ...
                     # ── 2026-06-15: PERP-IMB DEEP VETO (all tiers) ──────────
                     perp_imb_veto_enabled:    bool  = False,
                     perp_imb_veto_threshold:  float = -0.50,
                     # ── 2026-06-17: GOLDEN NEAR + HIGH-VOL VETO ─────────────
                     ...
                     # ── 2026-06-22: HARD ENTRY-PRICE FLOOR (all tiers) ───
                     # Final pre-order veto: never enter below this price.
                     min_entry_price:              int   = 0,
                     # ── 2026-06-17: MAX PER-TRADE CAPITAL CAP ───────────────
                     ...
                     # ── 2026-06-15: SURE-TRADE EV WALK-UP OVERRIDE ──────────
                     ...
                     # ── 2026-06-18: PREDICT-CROSS-EXIT (drift-aware) ────────
                     ...
                     # ── 2026-06-24: HOLD-TO-WIN (cancel resting TP on conviction) ─
    ...
    log.info(f"  CV quote-stability veto: {'ENABLED' if cv_stab_veto_enabled else 'DISABLED'}  "
             f"(block when Kalshi repricing momentum \u2264 -{cv_stab_mom_against:.0f} against side)")
    ...
    if consensus_veto_enabled:
        log.info(f"  Consensus veto: ENABLED  (binance_futures+OKX both must show "
                 f"|move|≥{consensus_min_move_pct:.3f}% opposing direction)")
        if consensus_smart_bypass:
            log.info(f"    Smart bypass: ENABLED — veto skipped when ANY of:")
            log.info(f"      (1) longer-window futures (≥{consensus_long_window_s:.0f}s) "
                     f"favors trade by ≥{consensus_long_min_pct:.3f}%  (6s blip is noise)")
            log.info(f"      (2) recent 5m bars sum favors trade by ≥{consensus_5m_favor_pct:.3f}%  "
                     f"(30-min trend overrides blip)")
            log.info(f"      (3) |dist_pct|≥{consensus_far_dist_pct:.3f}% AND "
    ...
    if hc_block_on_split_externals:
        log.info(f"  HIGH-CONV split-externals block: ENABLED — rejects HIGH-CONV "
    ...
    if last_bar_adverse_threshold > 0:
        log.info(f"  Last-bar adverse gate: ENABLED — block trades when LAST 5m bar opposes "
                 ...
                 f"skip when gap≥0.30 AND TP-direction ≥{high_conv_tp_strong:.2f} "
                 f"(or ≤{1-high_conv_tp_strong:.2f}) — in lock-in markets the very 5m bar "
                 f"that created the consensus often trips this gate.")
    else:
        log.info(f"  Last-bar adverse gate: DISABLED")
    if orderbook_signal_enabled:
        log.info(f"  Kalshi orderbook signal: RECORDING (audit-only, 5s cache). "
                 f"Captures depth, spread, imbalance per poll for retrospective analysis.")
    if trade_flow_signal_enabled:
        log.info(f"  Kalshi trade-flow signal: RECORDING (audit-only, last {trade_flow_lookback_n} "
    ...
    if sl_enabled:
        ...
        log.info(f"  Stop-loss: ENABLED  exit if {sl_trigger_mode}-price drops "
                 f"{sl_loss_cents}¢ below entry (STANDARD/strong/late-dir), "
                 f"{sl_loss_cents_high_conv}¢ (HIGH-CONV + LATE-SURE — high-price tiers) — "
        ...
        log.info("  Stop-loss: DISABLED — positions hold until settlement")
    if hedge_enabled:
    ...
    if take_profit_enabled:
        ...
        # Ceiling TP works without the +N¢ TP being enabled.
    ...
    if bid_stab_veto_enabled:
        log.info(f"  Bid-stability entry veto: ENABLED  — skip entry unless our "
                 f"side's Kalshi bid is stable/rising over the last "
    ...
    if bid_stab_veto_enabled and bid_stab_burst_samples > 0:
        log.info(f"  Bid-stability BURST confirm: {bid_stab_burst_samples} live "
                 ...
                 f"real-time window) — any downtick vetoes + re-polls")
    ...
    if perp_imb_veto_enabled:
        log.info(f"  Perp-imb deep veto: ENABLED (all tiers)  \u2014 skip entry when "
                 f"Kalshi perp book imbalance \u2264 {perp_imb_veto_threshold:+.2f} "
    ...
    if min_entry_price > 0:
        log.warning(f"  Entry-price FLOOR: ENABLED — never enter below "
                    f"{min_entry_price}¢ (all tiers). Keeps >=89¢ deep/high-conv "
    ...
    if ev_gate:
        log.info(f"  EV gate: ENABLED  (floor=${ev_floor:.2f}/c, ceiling={ev_ceiling}¢, "
                 f"TP weight={ev_tp_weight:.2f})")
        strong_hurst_tag = " + HURST-BYPASS" if strong_floor_hurst_bypass else ""
        log.info(f"           strong-floor=${ev_strong_floor:+.2f}/c when gap≥{ev_strong_gap_min:.2f} "
                 f"AND price≤{ev_strong_price_max}¢ AND TP≥{ev_strong_tp_min:.2f}/≤{1-ev_strong_tp_min:.2f} (directional) "
                 f"AND mins_left≤{ev_strong_max_mins:.1f} AND recent 6-bar momentum not >{ev_strong_max_adverse_momentum:.2f}% adverse"
                 f"{strong_hurst_tag}")
        ...
        log.info(f"           LATE-WINDOW SURE: floor=${late_window_ev_floor:+.2f}/c, "
                 f"price≤{late_window_price_max}¢ "
                 f"when mins_left≤{late_window_mins:.1f} AND TP≥{late_window_min_tp:.2f} "
        ...
        log.info(f"           HIGH-CONVICTION: floor=${high_conv_ev_floor:+.3f}/c (positive-EV only), "
                 f"price≤{high_conv_price_max}¢, mins_left≤{high_conv_max_mins:.1f} "
                 f"when gap≥{high_conv_gap_min:.2f} AND persist≥{high_conv_persist_min:.2f} "
                 f"AND TP extreme≥{high_conv_tp_strong:.2f}  (pure Kelly; can bypass timing)")
        if high_conv_vol_bypass:
            log.info(f"           HIGH-CONV vol bypass: enabled when "
                     f"|dist from strike|≥{high_conv_vol_bypass_distance:.2f}% AND "
                     f"(5m return≥{high_conv_vol_bypass_momentum:.2f}% in dir OR "
                     f"|dist|≥{high_conv_vol_bypass_strong_distance:.2f}% strong)")
        else:
        ...
        if late_dir_enabled:
            log.info(f"           LATE-DIR (late-window directional): floor=${late_dir_ev_floor:+.2f}/c, "
                     f"price≤{late_dir_price_max}¢ when mins<{late_dir_mins:.1f}, gap≥{late_dir_gap_min:.2f}, "
                     f"persist≥{late_dir_persist_min:.2f}, TP confirms direction, "
                     f"|dist|≥{late_dir_strong_distance:.2f}% (or momentum confirms)")
        else:
        ...
        log.info("  EV gate: disabled (flat 72¢ YES / 65¢ NO price caps in effect)")
    if retry_walk_cents > 0:
        log.info(f"  Order retry: walk up to +{retry_walk_cents}¢ on zero-fill "
    ...
    if order_lead_cents > 0:
        log.info(f"  Order lead: submit initial order at +{order_lead_cents}¢ above "
    ...
    # If signal still approves on next poll, we re-attempt — but bounded by
    # max_window_fill_attempts to avoid spamming Kalshi if the orderbook is
    # persistently moving away from us.
    window_no_fills: dict[str, int] = {}
    # 2026-06-12: PATIENT TOP-UP — track the LARGEST intended size across all
    ...
    # Structured audit log — one JSONL file per UTC day in logs/audit_*.jsonl
    ...
    # poller feeds BOTH lead venues: futures_lead (binance_futures — primary,
    # replaces the old CME/Databento feed) and okx_feed (okx — secondary,
    ...
    # soft failure — all lead vetoes fail-open.
    ...
    if not (futures_lead_disabled and okx_disabled):
        if os.environ.get("CHAINVECTOR_API_KEY", ""):
            ...
            log.warning("CHAINVECTOR_API_KEY not set — futures-lead/OKX vetoes and "
    ...
    # power loss). Without this, a position held in Kalshi but not in
    ...
    if not dry_run:
        ...
        # ── 2026-06-15: RESTING-TP ORPHAN CLEANUP ────────────────────────────
        ...
        if resting_tp_enabled:
            try:
                ...
                log.warning(f"[RESTING-TP] startup orphan cleanup failed "
                            f"(non-fatal): {_e!r}")
            # ── RE-PLACE TPs FOR RECOVERED POSITIONS ─────────────────────────
        ...
        # ── TIGHT SL MONITOR FOR RECOVERED POSITIONS ─────────────────────────
        ...
        # each resolves (SL / full TP) or nears settlement — consistent with the
    ...
    while True:
        ...
        for wid, trade in list(session.pending.items()):
            # ── 2026-06-15: RECOVERED-POSITION RESTING-TP MONITOR ────────────
            ...
            if result is not None:
                ...
                if hedge_state and hedge_state.get("active"):
                    ...
                    if hedge_won:
                        hedge_pnl = h_net_win
                        # Subtract fees (rough estimate: 2× maker fee × 0.5)
                ...
                # ── 2026-06-01: flip leg settlement resolution ──────────
        ...
        # ── Kalshi orderbook + trade-flow snapshot (audit-only, 5s cached) ──
        ...
        # ── Terminal Probability (ChainVector six-estimator ensemble) ─────────
        ...
        # Use the most recent TP result for this window (always one — fresh if
        # we just computed, or up to ~60s old if not).
        tp_cached = window_tp_cache.get(window_id)

        # Status line — add term_p_yes with freshness tag
        tp_str = ""
        ...
        # ── (Optional) gate-relaxation logic — OFF by default ─────────────────
        ...
        # this poll only. Cleans only the gap blocker — other gates unchanged.
            ...
            if confirms and sig["gap"] >= 0.08 and "Markov gap" in " | ".join(reasons):
                ...
                if not new_reasons:
                    log.info(
                        ...
                        f"→ relaxing gap gate (was 0.11, allowing 0.08)"
                    )
                    ...
                    # by reusing the signal's contracts field — already > 0 if Markov
        ...
        # ── (Optional) EV gate override — OFF by default ──────────────────────
        ...
        # floor (default $0.05) AND we're below the safety ceiling (90¢
        # regardless of EV), we override the price-cap rejection. This
        # captures high-confidence trades that the flat 72¢/65¢ cap misses
        # when both forward-looking signals agree.
        #
        # Direction safety: if combined-p points OPPOSITE Markov, we abort
        # the override — never trade against the deterministic call.
        ...
        if ev_gate and not approved and reasons:
            ...
            # high-conv only — other tiers stay locked to the standard window.
            ...
            # Directional confirmation for vol bypass — independent of any
            ...
            # = "NO_TRADE" (a truthy string!) when the trade is rejected — not
            ...
            # Late-window directional vol bypass (new tier — looser than
            ...
            # strict requirements (gap≥0.30 + TP≥0.85 for high-conv, or
            # mins<5 + TP-direction-extreme for late-sure) — when met, the
            ...
            if tp_cached_now is not None and not tp_cached_now.get("error"):
                ...
                if tp_p is not None:
                    ...
                    # 3-min binary probability; insisting on TP≥0.85 made
                    ...
                    late_sure_qualifies = (
                        sig["minutes_left"] <= late_window_mins  # 2026-05-28 PM: < → <=
                        and tp_extreme_ls_or_lockin
                        and (sig["p_yes"] > 0.5) == rec_yes
                    )
                    # 2026-05-28 PM3: strong-floor can ALSO bypass Hurst
                    # when its own gates (gap≥0.13, price≤88, TP-meaningful,
                    # mins≤8, direction match) all hold AND the user opts in.
                    # Note: strong-floor's EV floor (-$0.06 default) still
                    # applies — won't fire unless EV passes. Combine with
            ...
            if qualifies_for_eval:
                ...
                # BOTH temporal halves — unlike the book/flow/liq composites,
                ...
                # Markov's IMPLIED direction (from p_yes) — `rec` is "NO_TRADE"
                ...
                if new_rec != markov_implied:
                    log.info(
                        ...
                        f"— direction conflict, skip override"
                    )
                else:
                    ...
                    # above/below 50%. Loss on 2026-05-27 21:24 (YES @ 68¢,
                    # -$43): TP was 52.7% — barely above coin-flip — and the
                    # trade was a dead-cat bounce. Requiring TP ≥ 0.55 (or
                    # ≤ 0.45 for NO) filters out these marginal cases.
                    ...
                    # fired at 11-12 min left — classic "mid-market reversal"
                    # pattern. Capping strong-floor to mins_left ≤ 8 forces
                    ...
                    # disagree, we allow bidding much higher (default 89¢) at
                    ...
                    # ── Orderbook lock-in confirmation (2026-05-28 PM) ─────
                    ...
                    # raise LATE-SURE's effective cap by 1¢ (98 → 99). The
                    # orderbook is orthogonal evidence of market consensus
                    # at the absolute extreme — justifies the 1¢ extra risk.
                    ...
                    is_late_sure, late_reason = _is_late_window_sure(
                        ...
                        late_window_min_tp=late_window_min_tp,
                        # 2026-05-28 PM: lock-in confirmed → bypass TP
                    ...
                    # the ceiling to 97¢ but REQUIRES positive EV at that
                    # price (no negative-EV tolerance — risk:reward is 30+:1
                    ...
                    # ── HIGH-CONV split-externals block ──────────────────
                    # 2026-05-27 PM analysis: today's only HIGH-CONV loss
                    # caught by SL had lead-venue -0.01% / OKX +0.003% —
                    ...
                    # XOR logic — both-oppose is caught by consensus veto;
                    # both-agree or both-neutral pass through.
                    #
                    # Only applies to HIGH-CONV tier — STANDARD/strong/
                    ...
                    # confirmed. The lock-in pattern (spread≤2, top-of-book
                    # ≥85¢ in trade direction, gap≥0.20) is itself a third-
                    # source orthogonal confirmation — exactly what the split
                    # rule was added to compensate for. Stacking another
                    # opposition check on top blocks clean lock-in trades
                    # like 2026-05-28 23:38 NO @ 87¢ (db=+0.0102% = 1bp of
                    ...
                    #   late-sure   → TP-leaning p_win, lenient floor, cap 89
                    #   high-conv   → TP-leaning p_win, positive-EV only, cap 97
                    #   strong      → standard combined_p, mild lenient floor, cap 80
                    #   default     → standard combined_p, standard floor, cap 90
                    # Ordering rationale: late-sure has the most lenient floor
                    # (allows -$0.10 EV) so it wins below 89¢ if applicable;
                    # high-conv takes over for 89-97¢ when signals are extreme;
                    ...
                    #   • timing was NOT blocked (timing still requires HC), AND
                    #   • either no vol-block OR vol-block was already authorized
                    ...
                    # design), but still can't bypass timing — that requires
                    ...
                    if late_sure_can_fire:
                        if ob_lockin_confirmed:
                            # 2026-05-28 PM: lock-in path — use Markov-only
                        ...
                        # use the elevated cap (98 → 99 by default).
                        effective_ceiling = effective_late_window_price_max
                        active_tier       = "late_sure"
                    elif is_high_conv:
                        # 2026-05-28 PM:
                        #   • Standard HC path: boost p_win using TP (since
                        #     TP-extreme is required).
                        #   • Lock-in HC path: use Markov-only (or max with
                        ...
                        # p_win isn't applied here — strong-floor keeps its
                        # combined p_win since gap≥0.13 isn't "extreme enough"
                    ...
                    if active_tier == "none":
                        ...
                        log.info(
                            f"   EV-gate: price {limit_price}¢ exceeds "
                            f"{ceiling_tag} ceiling "
                            f"{effective_ceiling}¢ — skip "
                        ...
                        # for high-conv (the most common case at 98+¢).
                        if active_tier == "standard":
                            ...
                            if hc_ok_except_price and limit_price > high_conv_price_max:
                                log.info(
                                    f"   (high-conv conditions ALL met — "
                                    f"only price blocked: {limit_price}¢ > {high_conv_price_max}¢. "
                        ...
                        if is_late_sure:
                            ...
                            tier_detail = (f"  [high-conv{bypass_str}: gap={sig['gap']:.3f}≥{high_conv_gap_min:.2f}, "
                                          f"persist={sig.get('persist',0):.2f}, "
                                          f"TP={tp_str}, p_win={p_win:.3f}]")
                        elif is_late_dir:
                            tier_detail = (f"  [late-dir: mins={sig['minutes_left']:.1f}, "
                                          f"gap={sig['gap']:.3f}≥{late_dir_gap_min:.2f}, "
                                          f"persist={sig.get('persist',0):.2f}, "
                                          f"TP={tp_cached_now['bs_p_above']:.3f}]")
                        elif is_strong:
                            tier_detail = (f"  [strong: gap={sig['gap']:.3f}≥{ev_strong_gap_min:.2f}, "
                                          f"price≤{ev_strong_price_max}¢, TP confirms]")
                        else:
                            tier_detail = ""
                        log.info(
                            f"   EV-gate: EV ${ev_per:+.4f}/c below {floor_label} "
                            f"${floor_to_use:+.3f}/c at {limit_price}¢  ({src})"
                            + tier_detail
                        )
                    else:
                        # Kelly sizing tiers (same as run_signal) — anti-Kelly at high prices
                        if   65 <= limit_price <= 73: frac = 0.35
                        elif 73 <  limit_price <= 79: frac = 0.12
                        elif 79 <  limit_price <= 85: frac = 0.08
                        else:                         frac = 0.05
                        ...
                        #   • HIGH-CONV tier: set frac to high_conv_confirmed_frac
                        #     (default 0.10 = 2x baseline 0.05)
                        #   • Non-HC tiers (standard/strong/late-sure/late-dir):
                        ...
                        if okx_feed is not None:
                            try:
                                ...
                                # When CME (Databento) is stale — e.g. Fri-Sun
                                # close — fall back to OKX-only confirmation so
                        ...
                        # would default to the `max(1, …)` floor of one contract.
                        # Relaxed-floor tiers (strong-floor, late-sure) explicitly
                        # accept some negative EV — they should size to a small,
                        ...
                        # those signals — the elif `is_strong` matched first.
                        floor_stake_label = ""
        ...
        if not approved:
            ...
            # No API cost — both get_recent_move() calls read from in-memory
            ...
            # Re-poll if every blocker is "transient" — depends on instantaneous
            # market state and may flip before the entry window closes:
            #   • timing       — by definition ticks down each second
            #   • price cap    — orderbook moves continuously
            #   • near-strike  — BTC oscillation around the strike
            ...
            def _is_transient(r: str) -> bool:
                ...
                # GK volatility recomputes as new 5-min candles arrive — every
                ...
                if "mean-reverting" in r:
                    return mins_left > 5.0
                # blocked UTC hour, building history → terminal (truly invariant)
                return False
            ...
            if transient_only and mins_left > 3.5:
                ...
                # of Kalshi's 100/min limit — plenty of headroom.
        ...
        # ── ChainVector FUTURES LEAD VETO (binance_futures venue) ─────────────
        # Last check before placing the order: did CME MBT futures just move
        # AGAINST our direction? If so, skip — Kalshi is likely to follow.
        # Veto is soft: stale/missing data → don't veto (assume signal not
        ...
        if futures_lead is not None and not itm_lockin_exempt:
            ...
            if not db_check["consistent"]:
                log.warning(
                    f"FUTURES-LEAD VETO — skipping {rec} trade. {db_check['reason']}. "
                    f"({db_summary})"
                )
                ...
                log.info(f"   Futures-lead OK — {db_check['reason']}  ({db_summary})")

        # ── ChainVector MOMENTUM-SCORECARD VETO (strong-against) ─────────────
        ...
        if (cv_mom_veto_enabled and cv_lead_hub is not None
            ...
            if _aggv is not None:
                ...
                if _signed_mom <= -cv_mom_veto_score and _breadth_against:
                    log.warning(
                        f"CV-MOMENTUM VETO — scorecard strongly against {rec}: "
        ...
        # ── ChainVector PROBABILITY-ENGINE VETO (losing-market floor) ────────
        ...
        # blend: if the six-estimator ensemble says our side has ≤ N%
            ...
            if _tpv and not _tpv.get("error"):
                ...
                if isinstance(_p_above, (int, float)):
                    ...
                    if _p_side <= cv_prob_veto_max:
                        log.warning(
                            f"CV-PROB VETO — probability engine gives {rec} only "
        ...
        if (hurst_tp_veto_enabled and active_tier in hurst_tp_veto_tiers
            ...
            if (_h is not None and isinstance(_tp_p, (int, float))):
                ...
                if _h >= hurst_tp_veto_min_hurst and _adv_diff >= hurst_tp_veto_min_diff:
                    log.warning(
                        f"HURST+TP VETO — Hurst={_h:.2f}\u2265{hurst_tp_veto_min_hurst:.2f} "
        ...
        if (max_adverse_bar_veto_enabled and active_tier in max_adverse_bar_veto_tiers):
            ...
            if _rec5m_for_veto:
                ...
                if _max_adv_mag >= max_adverse_bar_veto_pct:
                    log.warning(
                        f"MAX-ADVERSE-BAR VETO — recent 5m bar moved "
        ...
        # ── CUMULATIVE-ADVERSE-MOMENTUM VETO ─────────────────────────────────
        ...
        # never repriced (bid stayed 84-96¢) so the SL never fired; BTC
        ...
        if (cum_adverse_momentum_veto_enabled
            ...
            if _rec5m_for_cum:
                ...
                if _adv_cum >= cum_adverse_momentum_veto_pct:
                    log.warning(
                        f"CUM-ADVERSE-MOMENTUM VETO — recent 5m sum drifted "
                        f"{_adv_cum:+.3f}% against {rec} over ~30 min "
                        f"(≥{cum_adverse_momentum_veto_pct:.3f}% threshold). "
                        f"Skipping {rec} {active_tier} trade to avoid "
        ...
        # signature:
        #
        #   YES @ 90¢ (HIGH-CONV)  Markov=0.95  TP=0.88
        #   binance_futures -0.01% (under 5bps threshold)
        #   OKX       -0.025% (record-only at the time)
        #   → BTC dropped through strike in 11 min, lost $21.40
        #
        ...
        if (consensus_veto_enabled and okx_feed is not None
            ...
            if db_opp and ok_opp:
                # ── 2026-06-01: Smart bypass checks ────────────────────────
                ...
                if consensus_smart_bypass:
                    ...
                    if db_long_move is not None:
                        # Favoring trade: YES → positive move, NO → negative move
                        if side_yes:
                    ...
                    if not bypass_active:
                        ...
                        if _dist_pct is not None:
                            ...
                            if (abs(_dist_pct) >= consensus_far_dist_pct
                                ...
                                bypass_reason = (f"|dist|={abs(_dist_pct):.3f}% ≥ "
                ...
                if bypass_active:
                    log.info(
                        f"CONSENSUS VETO BYPASSED ({bypass_kind}) — {bypass_reason}. "
                        f"Original veto: db={db_move:+.4f}%, okx={okx_move:+.4f}% "
                        f"both oppose {rec} (≥{consensus_min_move_pct:.3f}%)."
                    )
                    audit.write(build_poll_record(
                        window_id=window_id, ticker=ticker, close_dt=close_dt,
                    ...
                    # (do not set traded, do not continue/sleep — let the
                    # outer flow proceed to EV gate / tier / fire path).
                else:
                    log.warning(
                        f"CONSENSUS VETO — both externals oppose {rec} trade. "
                        f"Futures move={db_move:+.4f}%, OKX move={okx_move:+.4f}% "
                        f"(both |move| ≥ {consensus_min_move_pct:.3f}% directional threshold). "
                    ...
                    # to `session.traded` and slept until close — abandoning a
                    ...
                    if mins_left > 3.5:
                        ...
                        await asyncio.sleep(max(5, (mins_left + 0.5) * 60))
                    continue
                # else: bypass_active=True → fall through to normal trade flow
            elif db_opp or ok_opp:
                # Single-source opposing — log but don't veto (historical data
                # shows wins in this category).
                log.info(
                    f"   Consensus check: one source opposes "
                    f"(db={db_move}, okx={okx_move}) — single-source opposing "
                    f"not a veto trigger, proceeding."
                )

        # ── 2026-06-09: ROLLING-WR DEFENSIVE MODE GATE ───────────────────────
        ...
        # ── 2026-06-15: MIN-ENTRY-SIZE GATE (standard tier) ──────────────────
        ...
        # entry or a high-conv qualification later. Standard only — the sure
        ...
        if (_ms_tier == "standard"
            ...
            log.info(
                ...
                f"{standard_min_entry_contracts}c @ {limit_price}\u00a2 — skipping; "
        ...
        # single-ticket dollar risk regardless of tier/Kelly — clips only the
        # oversized monster tickets (the −$835/−$1070 near-strike reversals)
        ...
        # ── LOG-ONLY new-signal snapshot (ChainVector live signals) ──────────
        ...
        try:
            ...
            audit.write({
                ...
                **{k: _ns[k] for k in (
                ...
                # ticker plus regime/volatility/risk-index. Log-only for now —
        ...
        # Block when recent Kalshi taker aggression is strongly AGAINST the
        ...
        # rising over the recent lookback — a falling / off-peak bid means the
        ...
        if bid_stab_veto_enabled:
            ...
            if _bsv is None and bid_stab_burst_samples > 0:
                _bsv = await _bidstab_burst(rec, bid_stab_burst_samples,
                                            ...
                                            window_id)
            if _bsv:
                log.warning(f"BID-STABILITY VETO — {_bsv}; skipping entry.")
                audit.write(build_poll_record(
                    window_id=window_id, ticker=ticker, close_dt=close_dt,
                    decision="NO_TRADE", rec=rec,
        ...
        # YES-mid repricing speed (−100..+100, positive = YES getting more
        # expensive). If the market is repricing hard AGAINST our side, the
        # crowd is exiting our direction at this exact moment — wait it out.
        ...
        if cv_stab_veto_enabled and _cv_client.enabled:
            ...
            if _stab_mom is not None:
                ...
                if _stab_signed <= -cv_stab_mom_against:
                    log.warning(
                        f"CV-STABILITY VETO — Kalshi quote repricing against {rec}: "
        ...
        # ── ChainVector LIQUIDATION CASCADE-RISK VETO ────────────────────────
        ...
        if cv_cascade_veto_enabled and _ns is not None:
            ...
            if isinstance(_casc, dict) and _casc.get("risk_score") is not None:
                ...
                if _crisk is not None and _crisk >= cv_cascade_veto_score and _against:
                    log.warning(
                        f"CV-CASCADE VETO — liquidation cascade risk {_crisk:.0f} "
        ...
        # ── 2026-06-12: PERP-MOMENTUM ENTRY VETO (ENFORCED) ──────────────────
        # Skip the trade when the Kalshi perp's 30s tape is actively moving
        ...
        # a single −$446 catastrophe (2026-06-12 13:56). Fail-open: if the
        ...
        if (perp_veto_enabled and _ns is not None
            ...
            log.warning(
                f"PERP VETO — perp 30s momentum {_ns['perp_m30s']:+.2f}bp "
            ...
            # Transient veto: the 30s blip may pass — re-poll if time permits.
            if mins_left > 3.5:
                await asyncio.sleep(10)
            else:
                session.traded.add(window_id)
        ...
        # ── 2026-06-14: BOOK-SKEW ENTRY VETO (golden-zone only) ──────────────
        ...
        # ChainVector orderbook-imbalance read — fail-open if the feed is
        ...
        if (book_skew_veto_enabled and _bs_applies and _ns is not None
            ...
            log.warning(
                f"BOOK-SKEW VETO — Binance futures depth skew "
        ...
        # ── 2026-06-15: PERP-IMB DEEP VETO (all tiers) ───────────────────────
        # Kalshi perp book depth imbalance (signed toward trade side). When the
        # perp book is STRONGLY stacked against the position (<= deep negative
        # threshold), it flags a reversal-prone subset (e.g. the −$248 NO at
        # perp_imb −0.73). Backtest: deep threshold (≤−0.50) net-positive on
        # all tiers, cuts only SMALL winners (no big winners ≤−0.50). Catches a
        ...
        if (perp_imb_veto_enabled and _ns is not None
            ...
            log.warning(
                f"PERP-IMB VETO — Kalshi perp book imbalance "
        ...
        # (|dist%|<0.08, GK_vol>=~0.0020) lost ~36% vs ~20% for far entries —
        ...
        # Transient re-poll like the other entry vetoes — keeps the window open
        ...
        if MIN_ENTRY_CUSHION_PCT > 0:
            ...
            if _mec_dist is not None and abs(_mec_dist) < MIN_ENTRY_CUSHION_PCT:
                log.warning(
                    f"MIN-CUSHION VETO — {rec} @ {limit_price}¢ only "
        ...
        # ── 2026-06-22: HARD ENTRY-PRICE FLOOR (all tiers, final pre-order) ──
        if min_entry_price > 0 and limit_price < min_entry_price:
            log.warning(
                f"ENTRY-PRICE FLOOR — {rec} @ {limit_price}¢ < floor "
                f"{min_entry_price}¢; skipping (cheap-tier reversal risk). "
        ...
        if dry_run:
            ...
            # Dry-run hedge logging — show what hedge WOULD have done
            if hedge_enabled:
            ...
            # ── 2026-06-01: Dry-run Smart FLIP simulation ─────────────────
            ...
            # past the cap — that would defeat the strategy's EV protection.
            #
            ...
            # trades where the orderbook is racing up faster than 1¢/poll
            # walks can chase.
            effective_lead = 0
            if order_lead_cents > 0:
                ...
                if lead_ev >= lead_floor:
                    ...
                    log.info(
                        f"   ORDER LEAD +{effective_lead}¢ — submitting at {limit_price}¢ "
                        f"(EV ${lead_ev:+.4f}/c ≥ floor ${lead_floor:+.3f}/c, "
                        f"p_win={lead_p_win:.3f})"
                    )
                else:
                    log.info(
                        f"   ORDER LEAD +{order_lead_cents}¢ BLOCKED — EV at "
                        f"{lead_price}¢ would be ${lead_ev:+.4f}/c, below "
            ...
            for chunk_idx in range(MAX_CHUNKS):
                ...
                try:
                    ...
                    log.error(f"Kalshi API error on chunk {chunk_idx+1}: "
                ...
                if chunk_filled == 0:
                    fill_log.append(
                        f"chunk#{chunk_idx+1}: 0/{this_chunk} (depth exhausted at {limit_price}¢)"
                    )
                    break
                ...
                # ── Phase 2: interleaved hedge top-up ──────────────────
                ...
                # but we can keep going — next chunk may catch newly-arrived bids/asks.
                # Loop will stop naturally on the FIRST 0-fill chunk.

            # ── Walking +N¢ retry on UNFILLED contracts ─────────────────────
            # 2026-05-29 BUG FIX: previously walked ONLY on zero fills. Partial
            # fills (e.g., chunk#1: 3/5, chunk#2: 0/5 — total 3/32) skipped the
            ...
            #   • full quantity filled (success)
            #   • EV at next price drops below floor (math says stop)
            #   • price would exceed the safety ceiling (hard stop)
            #   • we've walked the configured maximum (retry_walk_cents)
            walked = 0
            ...
            while (filled_count < contracts and not api_error
                ...
                if new_limit_price > retry_ceiling:
                    retry_stop_reason = f"price {new_limit_price}¢ > ceiling {retry_ceiling}¢"
                    log.info(f"   +{walked}¢ walk STOPPED: {retry_stop_reason}")
                    break
                ...
                # band with book_skew<=0 was strongly -EV — so book_skew is the
                ...
                if retry_ev < eff_floor:
                    tier_note = retry_tier_tag if retry_tier_tag else " (no tier qualifies at this price → standard floor)"
                    retry_stop_reason = (
                        f"EV at {new_limit_price}¢ is ${retry_ev:+.4f}/c, "
                ...
                if _ov_active and retry_ev < retry_floor:
                    ...
                    log.info(
                        f"   +{walked}¢ EV-OVERRIDE [sure+book]: walking to "
                ...
                log.info(
                    f"   +{walked}¢ WALK{retry_tier_tag} — trying {new_limit_price}¢ "
                ...
                try:
                    ...
                    if retry_filled > 0:
                        ...
                        fill_log.append(
                            f"+{walked}¢ walk: {retry_filled}/{remaining_to_fill} @ ${retry_avg:.4f}"
                        )
                        log.info(f"   +{walked}¢ walk FILLED — {retry_filled}/{remaining_to_fill}c at ${retry_avg:.4f} (cumulative {filled_count}/{contracts})")
                        ...
                        fill_log.append(
                            f"+{walked}¢ walk: 0/{remaining_to_fill} (orderbook empty at {new_limit_price}¢)"
                        )
                except Exception as e:
                    log.warning(f"+{walked}¢ walk submit failed: {e}")
            ...
            if filled_count <= 0:
                ...
                if retry_walk_cents > 0:
                    if walked > 0:
                        walk_msg = (f", walked {walked} cent(s)"
                                    + (f" — stopped: {retry_stop_reason}" if retry_stop_reason else ""))
                    else:
                ...
                if attempt_n >= max_window_fill_attempts:
                    log.warning(
                        f"NO FILLS — requested {contracts}c @ {limit_price}\u00a2{walk_msg}. "
                    ...
                    # Re-evaluate next poll. Don't mark window as traded — if
            ...
            # P&L math — matches the V2 API's string format.
            ...
            log.info(
                f"ORDER PLACED — {len(order_ids)} chunk(s), filled={filled_count}/{contracts}c "
        ...
        # fill price if Kalshi returned one — slightly tighter P&L accounting).
        if not dry_run:
            if avg_fill_price:
                try:
            ...
            # active_tier stays "" → default to "standard" for the saved
            # field.
            ...
            # ── 2026-05-31 Phase 2: SAFETY HEDGE FINAL TOP-UP ─────────────
            ...
            if hedge_state.get("eligible") and not dry_run:
                # Final top-up — allow up to 5 more chunks to catch up
                await _hedge_topup_inline(
                    hedge_state=hedge_state,
                    current_yes_filled=filled_count,
                    ticker=ticker,
        ...
        # ── Stop-loss monitor (15m-specific) ─────────────────────────────
        ...
        # — typically once-per-day-ish at our 92%+ WR. Without SL, a HIGH-
        # CONV trade at 90¢ that goes wrong loses the full $0.90/c × 25c
        # = ~$22 (seen on 2026-05-27). A 45¢ SL caps that loss to ~$11.
        #
        # Implementation: poll every sl_poll_interval_s, verify position
        # with Kalshi before SELL (defensive — prevents over-sell that
        ...
        if (sl_enabled and pos and filled_count > 0 and not dry_run):
            ...
            # a tighter stop (default 30¢) because their downside is
            # asymmetric: upside is capped at 100¢ - entry (e.g., 7¢ for a
            # 93¢ trade) while downside can be most of entry cost. Tighter
            # stop limits the rare losses without blocking winners (these
            # tiers rarely dip 30¢+ from entry and recover).
            #
            # LATE-SURE added 2026-05-28 after raising its cap 89→96¢: at
            # 96¢ entry, the wider 45¢ SL would mean a 45¢/c loss on the
            # rare bad trade. Tightening to 30¢ matches HIGH-CONV's
            ...
            # 5-second polls — the daemon never saw an intermediate price.
            ...
            log.info(
                f"  ── STOP-LOSS MONITOR active: entry={entry_cents}¢ {rec} "
                f"(tier={position_tier}), stop=-{effective_sl_cents}¢ trigger({sl_trigger_mode}), "
            ...
            # ── 2026-06-11: REVERSAL-RISK MONITOR (RRM) — LOG-ONLY ──────────
            ...
            # ── 2026-06-15: place RESTING TAKE-PROFIT order ──────────────────
            # On a perp-confirmed position, rest a GTC limit sell at entry+TP.
            # Kalshi fills it the instant the bid touches the level (captures
            ...
            # ── 2026-06-27: HIGH-RISK TIGHT-TP state. Rather than manage a
            ...
            async def _arm_highrisk_tp():
                ...
                if _ed is None or abs(_ed) > highrisk_tp_dist_max:
                    return
                _basis = await get_kalshi_position_avg_cents(pos["ticker"])
                if _basis is None:
                    _basis = entry_cents
                ...
                if tp_price > 99 or tp_price <= _basis or pos["contracts"] <= 0:
                    log.info(f"  [HIGHRISK-TP] no room to lock "
                             f"+{highrisk_tp_cents}\u00a2 (basis {_basis}\u00a2) — skip.")
                    return
                ...
                log.warning(
                    ...
                    f"{mins_remaining:.1f}min left) — kept alive to close")
            ...
            async def _place_resting_tp():
                ...
                Two target modes:
                  * HIGH-PRICE CEILING: entry >= high_price_tp_min_cents -> rest
                    ...
                    tiers, not perp-gated. Handles "sure" 89¢+ trades whose
                    +10¢ target would exceed 99¢.
                  * STANDARD +N¢: entry + take_profit_cents, gated by the
                    perp-confirmed rules.
                """
                if not resting_tp_enabled:
                    return False
                # 2026-06-15: base the target on Kalshi's ACTUAL average cost,
                ...
                try:
                    ...
                    # NOTE: reduce_only is NOT used — verified 2026-06-15 the API
                    ...
                    # SL sell re-querying Kalshi's live position (sells min).
                    # 2026-06-18: V2 migration (legacy POST /portfolio/orders → 410).
                    ...
                    if oid:
                        ...
                        _drift = (f" [kalshi avg {_basis}\u00a2 vs tracked "
            ...
            async def _cancel_resting_tp(reason="exit"):
                ...
                # treated as confirmed — that would risk a double-sell.
                ...
                if confirmed:
                    ...
                    log.error(f"  [RESTING-TP] COULD NOT CONFIRM cancel id="
                              f"{oid[:8]} ({reason}) — holding market sell to "
                ...
                if _hw_eg is not None and _hw_eed is not None:
                    ...
                    if abs(_hw_eg) >= holdwin_min_gap and _hw_ecush >= holdwin_min_dist_pct:
                        ...
                        log.warning(
                            ...
                            f"{_hw_ecush:+.3f}%>={holdwin_min_dist_pct}%) — skip "
            ...
            while True:
                ...
                # be sold in the final 2 min — but we still cancel the resting
                ...
                if mins_remaining < _disarm_floor:
                    ...
                    # never a bad late fill — keep it resting through the close so
                    ...
                    if (rrm_state.get("anchor_btc") and rrm_state.get("anchor_perp")
                        ...
                        if not pos.get("_rrm_shadow"):
                            log.info(
                                f"  SL-monitor: {mins_remaining:.1f}min left "
                                f"(< {_disarm_floor:.1f}min) — entering RRM SHADOW "
                                f"tail (log-only, no sells) until settle.")
                            pos["_rrm_shadow"] = True
                    else:
                        log.info(
                            f"  SL-monitor: {mins_remaining:.1f}min left "
                            f"(< {_disarm_floor:.1f}min disarm threshold) — "
                ...
                if window_id not in session.pending:
                    ...
                    log.info(f"  SL-monitor: position no longer in pending — done")
                    break
                ...
                # ── 2026-06-15: monitor the RESTING TP order's fills ─────────
                # Poll its status; reconcile any new fills into the position.
                # If it fully filled, the position is closed — done.
                if pos.get("resting_tp_id"):
                    try:
                        ...
                        if _st:
                            ...
                            if (pos["contracts"] <= 0
                                    ...
                                    or _rem == 0):
                                log.warning(f"  [RESTING-TP] fully filled — position closed.")
                                pos["resting_tp_id"] = None
                                if window_id in session.pending:
                                    del session.pending[window_id]
                                sl_exited = True
                                # ── 2026-06-16: RESTING-TP RE-ENTRY ──────────
                                ...
                                # align. This branch is the sole re-entry path —
                                ...
                                if tp_reentry_enabled:
                                    ...
                                    if _mins_left_reentry >= 2.5:
                                        ...
                                        log.warning(
                                            ...
                                            f"left — re-opening window {window_id} "
                ...
                # ── 2026-06-27: arm the conditional high-risk tight TP once the
                ...
                # ── 2026-06-11: RRM evaluation (log-only OR live exit) ───────
                    ...
                    try:
                        ...
                        if rrm["ok"]:
                            ...
                            if rrm["would_exit"]:
                                ...
                                # 2026-06-30: telemetry enrichment — log the
                                ...
                                if _rrm_live:
                                    ...
                                    log.warning(
                                        ...
                                        f"{pos_side} {pos_count}c: {rrm['summary']} — "
                ...
                # 2026-06-30: shadow tail is strictly log-only — skip EVERY live
                ...
                # ── 2026-06-24: HOLD-TO-WIN (cancel resting TP on conviction) ─
                # On golden/standard winners that are clearly working — already
                # in profit, wide live cushion, low modeled P(end-OTM) — cancel
                ...
                # the gain) the moment conviction deteriorates — cushion
                    ...
                    if not pos.get("_holdwin_active"):
                        ...
                        if _sure:
                            ...
                            if _conf:
                                ...
                                log.warning(
                                    ...
                                    f"+{_hw_profit}¢, dist={_hw_dist}, "
                                    f"P(OTM)={_hw_potm}) — TP cancelled, riding "
                                    f"to settle for full (100-{entry_cents})¢.")
                                audit.write({
                        ...
                        if _deteriorate and not pos.get("resting_tp_id"):
                            ...
                            log.warning(
                                ...
                                f"retrace={_retrace}¢ (peak "
                                f"{pos.get('_holdwin_peak')}¢ bid "
                                f"{current_sell_price}¢).")
                            audit.write({
                ...
                if pcross_triggered:
                    hit_stop = True
                # ── 2026-06-22: PER-TRADE DOLLAR STOP (golden/standard tail cap) ──
                ...
                # only — high_conv/late_sure keep their tight per-contract stop
                ...
                if (_ds_cap > 0 and pos_count > 0
                    ...
                    log.warning(
                        ...
                        f"(-{loss_cents}¢/c x {pos_count}c) ≥ cap "
                        f"${_ds_cap:.0f} — flattening position."
                    )

                # ── 2026-06-15: PERP-CONFIRMED TAKE-PROFIT ───────────────────
                # Resting TP at entry+take_profit_cents. Backtest: on perp-
                # momentum-confirmed trades (entry m30s>0) a +10c TP was net
                # +$221 (+$520 on strong perp) — it captures the early profit
                ...
                if (not hit_stop and take_profit_enabled
                    ...
                    # if the bid has receded below +TP — so the remainder can't
                    ...
                    if pos.get("tp_committed"):
                        if current_sell_price > entry_cents:
                            ...
                            log.warning(
                                ...
                                f"profitable) — locking whole position out of profit.")
                    ...
                    if (ft_move is not None
                        ...
                        if adverse_move >= futures_fast_exit_threshold_pct:
                            ...
                            log.warning(
                                ...
                                f"{adverse_move:+.4f}% ≥ threshold "
                                f"{futures_fast_exit_threshold_pct:.3f}%). "
                                f"Exiting before SL triggers."
                            )

                # ── 2026-06-03 NEW: FADE-BOUNCE dual-entry ──────────────────
                # When the bid for our side dips into the 40-55¢ band mid-window,
                ...
                if (not hit_stop and fade_bounce_enabled
                    ...
                    if in_band:
                        ...
                        if fb_sig_ok:
                            ...
                            # 1.5%-of-bankroll floor override the dollar cap —
                            ...
                            if fb_contracts >= 5:  # min meaningful size
                                log.info(
                                    ...
                                    f"{fb_entry_price}¢  "
                                    f"(primary entry {entry_cents}¢, "
                                    f"savings {entry_cents - fb_entry_price}¢/c, "
                ...
                if (patient_topup_enabled and not dry_run and not hit_stop
                    ...
                    if (_tu_ask > 0 and _tu_ask < _tu_ref
                        ...
                        try:
                            ...
                            # fresh entry — the top-up fires late in the window
                            ...
                            if tu_ok:
                                ...
                                if _tu_block is not None:
                                    log.info(
                                        ...
                                        f"position — will retry if signals re-align.")
                                    tu_ok = False
                            if tu_ok:
                                ...
                                # toward it — not just toward the frozen
                                ...
                                if tu_n >= 5:
                                    ...
                                    if tu_filled > 0:
                                        ...
                                        log.info(
                                            ...
                                            f"{tu_price}\u00a2 — position now "
                                            f"{pos['contracts']}c, blended entry "
                                            f"{entry_cents}\u00a2")
                                        # 2026-06-15: position grew — cancel &
                ...
                # ── 2026-06-15: CANCEL RESTING TP BEFORE ANY MARKET SELL ─────
                ...
                if pos.get("resting_tp_id"):
                    ...
                    pos_count = pos["contracts"]
                    if not _confirmed:
                        # Resting order may still be live — selling now risks a
                ...
                if not fast_exit_triggered and not rrm_exit_triggered and not tp_triggered:
                    log.warning(
                        f"  STOP-LOSS HIT [{pos_ticker}] tier={position_tier}: entry={entry_cents}¢, "
                        f"trigger({sl_trigger_mode})={trigger_price}¢, sell_bid={current_sell_price}¢, "
                        f"loss=-{loss_cents}¢/c (threshold={effective_sl_cents}¢) × {pos_count}c "
                        f"= -${loss_cents * pos_count / 100:.2f}, "
                        f"mins_remaining={mins_remaining:.1f}, "
                        f"mins_since_entry={mins_since_entry:.1f}"
                    )

                # ── Verify actual position with Kalshi BEFORE selling ──
                # Defensive: prevent over-sell that could create an
                # unintended short. If query fails, fall back to in-memory.
                actual_count = await get_kalshi_position_count(pos_ticker)
                if actual_count is None:
                    log.warning(
                        f"  SL [{pos_ticker}]: position query failed — "
                        f"falling back to in-memory count {pos_count}c"
                    )
                    sell_count = pos_count
                elif actual_count == 0:
                    log.warning(
                        f"  SL [{pos_ticker}]: Kalshi shows 0 contracts but pending "
                    ...
                    if sell_count < pos_count:
                        log.warning(
                            ...
                            f"Kalshi shows {actual_count}c — selling smaller {sell_count}c"
                        )
                ...
                # $0.99 bid for NO equivalent). Kalshi fills limit orders
                # at the BEST available price, so we get whatever bid
                # exists — guaranteed fill, no chasing the falling bid.
                #
                # Background: today's two SL events lost ~$22 of EXTRA
                # money to slippage. Bid fell 25-30¢ between trigger
                # observation and order fill while we re-tried at the
                # observed (moving) bid each time. Aggressive sell
                # eliminates this — single shot, fills at the best bid
                ...
                # Kalshi (not our limit). With aggressive mode the fill
                # will be at the best bid that existed, not at our 0.01/0.99
                # extreme.
                #
                # Safer fallback for aggressive mode: if Kalshi's response
                # doesn't include `average_fill_price` (rare but possible),
                # DON'T fall back to our extreme limit ($0.01/$0.99) — that
                # would dramatically mis-record P&L. Fall back to the
                # observed bid instead, which is closer to reality. The
                # actual cash on Kalshi reflects the true fill price either
                ...
                if not sell_avg_s and sl_aggressive_sell:
                    log.warning(
                        f"  SL-SELL [{pos_ticker}]: Kalshi didn't return average_fill_price; "
                        f"using observed bid {current_sell_price}¢ for P&L (actual fill may differ)"
                    )
                ...
                log.info(
                    ...
                    f"{sell_filled}/{sell_count}c at {actual_sell_cents}¢ (trigger was "
                    f"{trigger_price}¢)  realized_pnl=${realized_pnl:+.2f}"
                )
                ...
                # Full exit — but if a safety hedge is attached, KEEP the
                ...
                if hedge_state and hedge_state.get("active"):
                    ...
                    # ── Phase 5: post-SL HEDGE BID MONITOR ──────────────
                    ...
                    while True:
                        ...
                        # Stop monitoring close to settlement — let it ride
                        ...
                        if not sell_reason:
                            await asyncio.sleep(hedge_post_sl_poll_s)
                            continue
                        # Sell the hedge at observed bid (NOT aggressive — we want
                        ...
                        if h_sell_filled <= 0:
                            log.warning(
                                f"  Hedge SELL: 0 fills at {h_bid}\u00a2 — bid moved; retry"
                            )
                            await asyncio.sleep(hedge_post_sl_poll_s)
                            continue
                        # Compute hedge PnL
                    ...
                    # ── 2026-06-01: Smart Defensive FLIP (with retry loop) ───
                            ...
                            if not flip_elig:
                                ...
                                if smart_flip_require_futures_confirm and futures_lead is not None:
                                    ...
                                    if flip_db_move is None:
                                        ...
                                        # For NO primary that SL'd → flip side = YES → we want futures UP
                                        # For YES primary that SL'd → flip side = NO → we want futures DOWN
                ...
                # losing exit — a fresh entry can catch a genuine flip OR
                ...
                if (sl_reentry_enabled
                    ...
                    if _mins_left_slre >= 2.5:
                        ...
                        log.warning(
                            ...
                            f"complete with {_mins_left_slre:.1f}min left — "
                        ...
                        log.info(
                            ...
                            f"(< 2.5min) — not re-opening; letting window close.")
                sl_exited = True
                break

            if sl_exited:
                # Skip the long sleep — position already closed, move on
                continue

            # ── 2026-06-12: RRM LATE-WINDOW COVERAGE (log-only) ──────────────
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Kalshi BTC 15m autonomous trading daemon (ChainVector-enhanced). "
                    "Defaults match the canonical production run — running with no "
    ...
    # ── ChainVector signal gates (this program's native signal provider) ─────
    ...
    parser.add_argument("--no-cv-mom-veto", dest="cv_mom_veto_enabled", action="store_false",
                        ...
                        help="Disable the ChainVector momentum strong-against veto (ON by "
                             ...
                             "score is \u2264 -–cv-mom-veto-score with breadth confirming.")
    parser.add_argument("--cv-mom-veto-score", type=float, default=65.0,
                        help="Signed aggregate momentum score at/below which entry is "
                             "vetoed (default 65 — 'strongly against').")
    ...
    parser.add_argument("--no-cv-stab-veto", dest="cv_stab_veto_enabled", action="store_false",
                        ...
                        help="Disable the ChainVector prediction quote-stability veto (ON "
                             ...
                             "shows the Kalshi quote repricing hard against our side.")
    ...
    parser.add_argument("--no-consensus-veto", action="store_true",
                        help="Disable the binance_futures+OKX consensus veto. By default, when BOTH "
                             "external feeds independently show a directional move ≥0.005%% against "
    ...
    parser.add_argument("--consensus-okx-lookback-s", type=float, default=6.0,
                        help="OKX lookback window for the consensus veto in seconds "
                             "(default 6.0 — matches the futures-lead lookback).")
    # ── 2026-06-01: Smart consensus-veto bypass ───────────────────────────────
    # 24h audit (17 vetoes): 65% had recent 5m bars strongly favoring the
    # trade direction — the 6s blip was clearly counter-trend noise. Three
    ...
    parser.add_argument("--consensus-long-window-s", type=float, default=60.0,
                        help="Bypass-1: longer futures-lead window (seconds) checked against "
                             "the 6s blip. If this longer window shows ≥ "
    ...
    # ── Stop-loss (NEW 2026-05-27) ────────────────────────────────────────────
    # 15m-specific stop-loss with WIDE 45¢ default trigger. Designed to cap
    ...
    parser.add_argument("--no-sl", action="store_true",
                        help="Disable the stop-loss. Default: ENABLED with -45¢ trigger.")
    ...
    parser.add_argument("--sl-loss-cents-high-conv", type=int, default=99,
                        help="Stop-loss threshold for HIGH-CONV AND LATE-SURE tiers (default 30). "
                             "Tighter than STANDARD's 45¢ because these high-price tiers have "
                             "asymmetric downside: entries at 86-96¢ mean upside is capped at "
                             "4-14¢ while downside can be 70-90¢+. A loss past 30¢ means BTC has "
    ...
    parser.add_argument("--max-loss-per-trade-high-conv", type=float, default=75.0,
                        help="Hard per-position DOLLAR stop-loss for the HIGH_CONV tier only "
                             ...
                             "losers dipped $459/$606 — so a $400 cap clips 0 winners and truncates "
    ...
    # ── 2026-06-03: FADE-BOUNCE dual-entry tier ──────────────────────────────
    # Captures the "fade the false reversal" pattern: when the Kalshi bid for
    # our side dips into the 40-55¢ band mid-window, 85%% of those windows
    ...
    parser.add_argument("--sl-trigger-mode", type=str, default="mid",
                        ...
                        help="Price to use for SL trigger check. 'bid' (conservative — "
                             "matches actual exit price), 'mid' (default — average of bid+ask, "
    ...
    parser.add_argument("--no-sl-aggressive-sell", action="store_true",
                        help="Disable aggressive sell on SL exits. Default: ENABLED. "
                             ...
                             "$0.99 bid for NO) so Kalshi fills against ANY remaining bid in "
                             "the book — guarantees fill, prevents chasing a falling bid. "
    ...
    parser.add_argument("--no-hc-block-on-split", action="store_true",
                        help="Disable the HIGH-CONV split-externals block. Default: ENABLED. "
                             ...
                             "qualification — trade falls through to lower tiers (which usually "
                             "means NO_TRADE if price is too high for them). Today's data: "
                             "would have prevented -$50.46 SL loss and missed +$10.40 win = "
                             "+$40 net. Only affects HIGH-CONV tier — STANDARD/strong/late-* "
    ...
    parser.add_argument("--high-conv-confirmed-frac", type=float, default=0.10,
                        help="Kelly fraction applied to HIGH-CONV tier when BOTH "
                             ...
                             "baseline of 0.05 for 86¢+ trades. Historical 5/26-5/27 "
    ...
    parser.add_argument("--ev-strong-price-max",type=int,   default=88,
                        help="Max entry price (cents) to qualify for strong-floor (default 88, "
                             "raised from 85→88 on 2026-05-28). On choppy days price_cap is "
                             "the top blocker; widening to 88 lets walking retry fill at 86-88¢ "
    ...
    parser.add_argument("--ev-strong-tp-min",   type=float, default=0.55,
                        help="Minimum TP (Deribit terminal probability) directional confirmation "
                             ...
                             "must be ≥0.55; for NO trades TP must be ≤0.45 (= 1-0.55). Previously "
                             "the gate just required TP same-direction as Markov, which allowed "
                             "TP at 50.1%% to qualify — too marginal. 2026-05-27 21:24 loss had "
    ...
    parser.add_argument("--ev-strong-max-adverse-momentum", type=float, default=0.10,
                        help="Maximum adverse cumulative recent_5m_pct%% allowed for strong-floor "
                             ...
                             "had net -0.21%% over 6 bars but were YES trades — Markov caught a "
    ...
    parser.add_argument("--last-bar-adverse-threshold", type=float, default=0.10,
                        help="LAST 5-min bar adverse-direction threshold (default 0.10 = 0.10%%). "
                             ...
                             "2026-05-27 22:07 loss (NO @ 73¢): last bar was +0.14%% just as BTC "
                             "started recovering — this gate would have BLOCKED it. Treated as "
    ...
    parser.add_argument("--no-orderbook-signal", dest="orderbook_signal_enabled",
                        action="store_false",
                        help="Disable Kalshi orderbook depth recording (default ON, audit-only). "
                             "Captures top-of-book + depth + imbalance per poll for retrospective "
                             "analysis. Added 2026-05-28 — after 1-2 days of data we can validate "
    ...
    parser.add_argument("--no-trade-flow-signal", dest="trade_flow_signal_enabled",
                        action="store_false",
                        help="Disable Kalshi recent-trades recording (default ON, audit-only). "
    ...
    parser.add_argument("--trade-flow-lookback-n", type=int, default=20,
                        help="Number of recent Kalshi trades to summarize per poll (default 20). "
    ...
    parser.add_argument("--taker-flow-veto-enabled", action="store_true", default=True,
                        help="Block entries when recent Kalshi taker aggression is at or "
    ...
    parser.add_argument("--late-window-price-max", type=int,   default=98,
                        help="Max bid price (¢) under late-window-sure tier (default 98). "
                             "2026-05-28: raised 96 → 98 after 3-day audit found EVERY late-window "
                             "lock-in candidate (104 polls, mins<5 with TP+Markov agreed) was "
                             "asking 97-100¢. Cap at 96 made LATE-SURE unreachable. With the "
                             "-$0.10/c EV floor an entry at 98¢ self-filters to p_win ≥ 0.90.")
    ...
    parser.add_argument("--no-orderbook-lockin", dest="orderbook_lockin_enabled",
                        ...
                        help="Disable orderbook-confirmed lock-in bypasses (default ON, 2026-05-28 PM). "
                             ...
                             "confirmation from Kalshi\u2019s own microstructure compensates for Deribit\u2019s "
    ...
    parser.add_argument("--hc-lockin-ev-floor", type=float, default=-0.10,
                        help="HIGH-CONV EV floor ($/c) when orderbook lock-in is confirmed (default -0.10, "
                             ...
                             "HC floor (default -0.05 via --high-conv-ev-floor) is too conservative — the "
    ...
    parser.add_argument("--late-window-min-tp",    type=float, default=0.75,
                        help="TP probability threshold confirming direction (default 0.75, "
                             ...
                             "out — BS-based TP applied to a 3-min binary structurally undershoots "
                             "true probability. Audit confirmed TP_bs clusters at 0.65-0.80 in the "
                             "late window even when Kalshi markets show 85-98%% consensus. The 0.85 "
    ...
    parser.add_argument("--golden-price-lo", type=int, default=65,
                        help="Golden-zone price band LOW (¢). Default 65. "
                             "Backtest favors 55.")
    parser.add_argument("--golden-price-hi", type=int, default=73,
                        help="Golden-zone price band HIGH (¢). Default 73. "
    ...
    parser.add_argument("--golden-no-dist", action="store_true",
                        help="Drop the near-strike distance gate FOR golden-zone "
                             "entries (the 65-73¢ band is itself the conviction "
    ...
    parser.add_argument("--high-conv-gap-min",     type=float, default=0.40,
                        help="Min Markov gap for high-conviction tier (default 0.40 per the "
                             "canonical baseline — strict HC filter, ~70/30 Markov confidence).")
    ...
    parser.add_argument("--high-conv-tp-strong",   type=float, default=0.90,
                        help="TP probability threshold confirming direction for high-conv "
                             "(default 0.90 → TP ≥ 90%% for YES or ≤ 10%% for NO). "
    ...
    parser.add_argument("--high-conv-price-max",   type=int,   default=97,
                        help="Max bid (¢) under high-conviction tier (default 97)")
    ...
    parser.add_argument("--high-conv-vol-bypass-strong-distance", type=float, default=0.25,
                        help="If |BTC-strike|/strike * 100 ≥ this %%, skip the 5m momentum check "
    ...
    parser.add_argument("--late-dir-momentum-min",    type=float, default=0.05,
                        help="Min |recent 5m return|%% in trade direction (default 0.05). "
                             "Skipped when distance ≥ strong-distance threshold.")
    parser.add_argument("--late-dir-strong-distance", type=float, default=0.08,
                        help="If distance ≥ this %%, skip 5m momentum check (default 0.08)")
    ...
    parser.add_argument("--patient-topup-dynamic-kelly", action="store_true",
                        help="Let the patient top-up grow the position toward "
                             ...
                             "Only ever adds when fresh EV>0 — so a trade that "
    ...
    parser.add_argument("--resting-tp-enabled", action="store_true", default=True,
                        help="ON by default in the canonical baseline. "
                             ...
                             "the gate/threshold. HIGH-RISK live order mgmt — "
    ...
    parser.add_argument("--tp-reentry-enabled", action="store_true",
                        help="After a resting-TP order FULLY sells (whole "
                             ...
                             "still align. Fires ONLY on a complete sell — never "
                             "on partial fills, SL/RRM exits, flips, or "
    ...
    parser.add_argument("--highrisk-tp-dd-cents", type=int, default=12,
                        help="Min drawdown (¢ below entry) to flag a high_conv "
    ...
    parser.add_argument("--highrisk-tp-cents", type=int, default=5,
                        help="Tight TP offset (¢ above entry) to lock in. Default 5.")
    # ── 2026-06-12: PERP-MOMENTUM ENTRY VETO ─────────────────────────────────
    ...
    parser.add_argument("--bid-stab-veto-enabled", action="store_true", default=True,
                        help="Skip entries unless our side's Kalshi bid "
    ...
    parser.add_argument("--book-skew-all-tiers", action="store_true",
                        help="Apply the book-skew veto to ALL tiers, not just "
                             "golden. NOT recommended — backtest shows it hurts "
                             "non-golden tiers.")
    # ── 2026-06-15: PERP-IMB DEEP VETO (all tiers) ───────────────────────────
    parser.add_argument("--perp-imb-veto", action="store_true",
                        help="Skip entries when the Kalshi perp book imbalance "
                             "(signed toward trade side) is <= threshold — i.e. "
    ...
    parser.add_argument("--perp-imb-veto-threshold", type=float, default=-0.50,
                        help="Signed perp_imb at or below which entry is vetoed. "
                             "Default -0.50 (deep — conservative on winner-cuts).")
    # ── 2026-06-17: GOLDEN NEAR + HIGH-VOL VETO ──────────────────────────────
    ...
    # ── 2026-06-24: HOLD-TO-WIN (cancel resting TP on high-conviction) ───
